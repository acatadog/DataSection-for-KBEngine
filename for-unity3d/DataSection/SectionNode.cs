...
namespace DataSection {
		...
		where T : SectionNode<T>, new()
	{
		...
		public SectionNode( string name, string value, T parent ) : base( name, value )
		{
			parent_ = parent;
		}
		...
		private T getSection_( string path, bool createIfNotExisted )
		{
			string[] p = path.Split( '/' );
			T section = (T)this;
		...
		{
			T section = getSection_( path, false );
			if (section != null)
